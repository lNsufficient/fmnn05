...
def state_events(t, y, yp, sw):
    ...
    if sw[0]:   
        print("in sw = 1")
        if phip_b < 0 and  h_s*phi_s < -( r_s -  r_0):
            new_state = 2
        if phip_b > 0 and  h_s*phi_s > ( r_s -  r_0):
            new_state = 3

    elif sw[1] and lambda_1_old*lambda_1 < 0: #self.current_state 
        new_state = 1
   ...
   # elif self.current_state == 4:
    ...
    lambda_1_old = lambda_1 #vi kräver att detta körs varje varv, annars får vi problem
    if new_state == -1:
        new_sw = sw
    else:
        new_sw = numpy.zeros(4)        
        new_sw[new_state-1] = 1
    print("at end of state_event")
...
problem = Implicit_Problem(res, y0, yd0, t0, sw0)
variables(y0)
